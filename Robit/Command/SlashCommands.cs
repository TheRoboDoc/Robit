...
namespace Robit.Command
{
    internal class SlashCommands : ApplicationCommandModule
    {
        ...
        [SlashCommandGroup("NewVegas", "MISTER NEW VEGAS")]
        [SlashCommandPermissions(Permissions.UseVoice)]
        public class NewVegas
        {
            [SlashCommand("Play", "Play New Vegas Radio")]
            public static async Task Play(InteractionContext ctx)
            {
                Program.BotClient?.Logger.LogDebug("OwO");
...
#pragma warning disable CS8604 // Possible null reference argument.
                ...
                AudioPlayer audioPlayer = new("./Audio/", channel, ctx.Channel);

                if (Program.AudioPlayers.Where(player => player.Channel == audioPlayer.Channel).Any())
                {
                    await ctx.CreateResponseAsync($"Already playing in {channel.Mention}", true);
                    return;
                }

                _ = Task.Run(audioPlayer.Play);

                await ctx.CreateResponseAsync($"Starting player in {channel.Mention}");

                Program.AudioPlayers.Add(audioPlayer);
            }

            [SlashCommand("Skip", "Skip to next song")]
...
#pragma warning disable CS8625 // Cannot convert null literal to non-nullable reference type.
                ...
                AudioPlayer? audioPlayer = Program.AudioPlayers.Where(
                    player => player.Channel.Id == channel.Id).FirstOrDefault();

                if (audioPlayer == null)
                {
                    await ctx.CreateResponseAsync("Nothing is playing in the channel where you are at", true);
                    return;
                }

                await ctx.CreateResponseAsync("Skipping...");
            ...
            public static async Task Leave(InteractionContext ctx)
            {
                ...
                await ctx.CreateResponseAsync($"Leaving {channel.Mention}");

                AudioPlayer audioPlayer = Program.AudioPlayers.Where(player => player.Channel == channel).First();

                Program.BotClient?.Logger.LogDebug("Got audio player");

                await audioPlayer.Disconnect();

                Program.BotClient?.Logger.LogDebug("Done disconnecting");

                Program.AudioPlayers.Remove(audioPlayer);
            }

            [SlashCommand("Pause", "Pause the current play")]
